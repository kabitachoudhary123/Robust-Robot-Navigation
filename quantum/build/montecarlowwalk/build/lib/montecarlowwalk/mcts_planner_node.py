...
class MCTSPlanner:
    ...
    def simulate(self, start: Tuple[int, int], goal: Tuple[int, int]) -> float:
        ...
        start_to_goal_dist = math.hypot(goal[0] - start[0], goal[1] - start[1])
        min_distance = start_to_goal_dist
        ...
        while tuple(current) != goal and steps < self.max_simulation_steps:
            ...
            possible_next_states.sort(key=heuristic)
            
            if random.random() < 0.7:
                next_state = possible_next_states[0]
            else:
                next_state = random.choice(possible_next_states[:3])
            
            current = list(next_state)
            visited_in_sim.add(tuple(current))
            
            current_dist = math.hypot(goal[0] - current[0], goal[1] - current[1])
            min_distance = min(min_distance, current_dist)
            
            progress_reward = (start_to_goal_dist - current_dist) * 2.0
        ...
        if tuple(current) == goal:
            ...
            final_distance = math.hypot(goal[0] - current[0], goal[1] - current[1])
            distance_reward = (start_to_goal_dist - final_distance) * (self.goal_reward / start_to_goal_dist)
            total_reward += distance_reward
            total_reward -= (final_distance * 0.1)
...
class MCTSPlannerNode(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('mcts_max_iterations', 30000),
                ('mcts_max_simulation_steps', 1000),
                ('mcts_exploration_weight', 1.5),
                ('path_chunk_length', 5.0),
                ('recovery_attempts', 5)
            ]
        )
    ...
    def _plan_next_path_chunk_async(self):
        try:
            ...
            chunk_length_cells = int(self.get_parameter('path_chunk_length').value / self.map_resolution)
            max_attempts = self.get_parameter('recovery_attempts').value
            sub_goal_found = False
            
            for attempt in range(max_attempts):
                attempt_chunk_length = chunk_length_cells * (max_attempts - attempt) / max_attempts
                
                candidate_points = []
                base_angle = math.atan2(goal_gy - current_gy, goal_gx - current_gx)
                
                for angle_offset in [-0.5, 0, 0.5]:
                    angle = base_angle + angle_offset
                    sgx = int(current_gx + attempt_chunk_length * math.cos(angle))
                    sgy = int(current_gy + attempt_chunk_length * math.sin(angle))
                    
                    sgx = max(0, min(sgx, self.mcts_planner.grid_width - 1))
                    sgy = max(0, min(sgy, self.mcts_planner.grid_height - 1))
                    
                    if self.mcts_planner.is_valid_state((sgx, sgy)):
                        candidate_points.append((sgx, sgy))
                
                for sgx, sgy in candidate_points:
                    ...
                    if path_chunk and path_chunk[-1] == (sgx, sgy):
                        with self.path_lock:
            ...
            if not sub_goal_found:
                self.get_logger().warn("Failed to find any valid path chunk after multiple attempts.")
                with self.path_lock:
                    self.path_buffer = []
    ...
    def plan_and_move_callback(self):
        try:
            ...
            if hasattr(self, 'last_position'):
                dist_moved = math.hypot(current_wx - self.last_position[0], 
                                       current_wy - self.last_position[1])
                if dist_moved < 0.05 and not self.planner_active_flag:
            ...
            self.last_position = (current_wx, current_wy)
