...
class QuantumMCTSPlanner:
    ...
    def a_star_search(self, start: Tuple[int, int], goal: Tuple[int, int]) -> List[Tuple[int, int]]:
        ...
        while open_set:
            ...
            if current == goal:
                ...
                path.append(start)
                self.get_logger().info("A* found a global path.")
                return path[::-1]
        ...
        self.get_logger().warn("A* failed to find a global path.")
        return []
    ...
    # --- MODIFIED: The main search is now a hybrid planner ---
    def search(self, root_state: Tuple[int, int], goal_state: Tuple[int, int], max_iterations: int, max_planning_time: float) -> List[Tuple[int, int]]:
        """
        First, runs A* to find a global path. If successful, it returns that path.
        If A* fails, it falls back to the robust MCTS search as a backup.
        """
        # 1. Try the fast, deterministic A* planner first.
        a_star_path = self.a_star_search(root_state, goal_state)
        if a_star_path:
            return a_star_path # If A* finds a path, we are done! It's fast and optimal.

        # 2. If A* fails (e.g., goal is in a small disconnected area), fall back to MCTS.
        self.get_logger().warn("A* failed. Falling back to Monte Carlo Tree Search. This may be slow.")
        self.simulation_cache = {}
