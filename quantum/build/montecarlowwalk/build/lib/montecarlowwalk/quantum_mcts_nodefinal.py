...
class QuantumMCTSPlannerNode(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                # --- NEW TUNING PARAMETER FOR SIMULATION QUALITY ---
        ...
        self.get_logger().info("Quantum MCTS Planner Node started.")
    ...
    def map_callback(self, msg: OccupancyGrid):
        ...
        # --- PASSING THE NEW PARAMETER TO THE PLANNER ---
    ...
    def goal_callback(self, msg: PoseStamped):
        ...
        if not self.mcts_planner or not self.robot_current_pose:
            self.get_logger().warn("Planner not ready (map or pose unavailable). Ignoring goal.")
            return

        self.current_goal = (msg.pose.position.x, msg.pose.position.y)
        goal_wx, goal_wy = self.current_goal
        map_min_wx, map_min_wy = self.map_origin
        map_max_wx = map_min_wx + self.map_width * self.map_resolution
        map_max_wy = map_min_wy + self.map_height * self.map_resolution

        if not (map_min_wx <= goal_wx < map_max_wx and map_min_wy <= goal_wy < map_max_wy):
            self.get_logger().error(f"Invalid Goal: Position ({goal_wx:.2f}, {goal_wy:.2f}) is outside the map boundaries.")
            self.current_goal = None
            return

        self.get_logger().info(f"Received goal: ({self.current_goal[0]:.2f}, {self.current_goal[1]:.2f}). Starting search...")
        
        start_pos = self.robot_current_pose.position
        start_grid = self.world_to_grid(start_pos.x, start_pos.y)
        goal_grid = self.world_to_grid(*self.current_goal)

        if not self.mcts_planner.is_valid_state(goal_grid):
            self.get_logger().error(f"Invalid Goal: Position is inside an inflated obstacle.")
            self.current_goal = None
            return
            
        distance_to_goal = math.hypot(self.current_goal[0] - start_pos.x, self.current_goal[1] - start_pos.y)
        ...
        if iters_per_meter <= 0:
            self.get_logger().warn(f"'iterations_per_meter' parameter is invalid ({iters_per_meter}). Falling back to default.")
            iters_per_meter = 2500
